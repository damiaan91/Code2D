 ...
 * </ul>
 */
public class C2dBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new nl.utwente.apc.Code2D.resource.c2d.mopp.C2dMetaInformation().getSyntaxName();
	
	private nl.utwente.apc.Code2D.resource.c2d.ui.C2dBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public C2dBracketPreferencePage() {
		super();
		
		nl.utwente.apc.Code2D.resource.c2d.IC2dMetaInformation metaInformation = new nl.utwente.apc.Code2D.resource.c2d.mopp.C2dMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(nl.utwente.apc.Code2D.resource.c2d.ui.C2dUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new nl.utwente.apc.Code2D.resource.c2d.ui.C2dBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					nl.utwente.apc.Code2D.resource.c2d.IC2dBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					nl.utwente.apc.Code2D.resource.c2d.IC2dBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + nl.utwente.apc.Code2D.resource.c2d.ui.C2dPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof nl.utwente.apc.Code2D.resource.c2d.ui.C2dEditor) {
			((nl.utwente.apc.Code2D.resource.c2d.ui.C2dEditor) editor).invalidateTextRepresentation();
		}
	}
