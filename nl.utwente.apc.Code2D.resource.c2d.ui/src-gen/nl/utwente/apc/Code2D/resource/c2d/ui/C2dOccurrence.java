 ...
 */
public class C2dOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "nl.utwente.apc.Code2D.resource.c2d.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "nl.utwente.apc.Code2D.resource.c2d.ui.occurences.declaration";
	...
	private nl.utwente.apc.Code2D.resource.c2d.ui.C2dTokenScanner tokenScanner;
	 ...
	 */
	public C2dOccurrence(nl.utwente.apc.Code2D.resource.c2d.IC2dTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, nl.utwente.apc.Code2D.resource.c2d.ui.C2dTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new nl.utwente.apc.Code2D.resource.c2d.mopp.C2dMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(nl.utwente.apc.Code2D.resource.c2d.ui.C2dBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, nl.utwente.apc.Code2D.resource.c2d.ui.C2dPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, nl.utwente.apc.Code2D.resource.c2d.ui.C2dPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, nl.utwente.apc.Code2D.resource.c2d.ui.C2dPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == nl.utwente.apc.Code2D.resource.c2d.ui.C2dPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(nl.utwente.apc.Code2D.resource.c2d.ui.C2dOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(nl.utwente.apc.Code2D.resource.c2d.ui.C2dOccurrence.DECLARATION_ANNOTATION_ID);
	}
