 ...
 * contains <code>nl.utwente.apc.Code2D.resource.c2.IC2TextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class C2MarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = nl.utwente.apc.Code2D.resource.c2.mopp.C2Plugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<nl.utwente.apc.Code2D.resource.c2.IC2Command<Object>> commands = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.c2.IC2Command<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(nl.utwente.apc.Code2D.resource.c2.IC2Command<Object> command) {
		...
		public void runCommands() {
			java.util.List<nl.utwente.apc.Code2D.resource.c2.IC2Command<Object>> commandsToProcess = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.c2.IC2Command<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (nl.utwente.apc.Code2D.resource.c2.IC2Command<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, nl.utwente.apc.Code2D.resource.c2.IC2TextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final nl.utwente.apc.Code2D.resource.c2.IC2TextDiagnostic diagnostic) {
		final nl.utwente.apc.Code2D.resource.c2.IC2Problem problem = diagnostic.getProblem();
		nl.utwente.apc.Code2D.resource.c2.C2EProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new nl.utwente.apc.Code2D.resource.c2.IC2Command<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == nl.utwente.apc.Code2D.resource.c2.C2EProblemSeverity.ERROR) {
					...
					nl.utwente.apc.Code2D.resource.c2.IC2TextDiagnostic textDiagnostic = (nl.utwente.apc.Code2D.resource.c2.IC2TextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof nl.utwente.apc.Code2D.resource.c2.mopp.C2Resource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((nl.utwente.apc.Code2D.resource.c2.mopp.C2Resource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<nl.utwente.apc.Code2D.resource.c2.IC2QuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, nl.utwente.apc.Code2D.resource.c2.util.C2StringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (nl.utwente.apc.Code2D.resource.c2.C2EProblemType nextType : nl.utwente.apc.Code2D.resource.c2.C2EProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, nl.utwente.apc.Code2D.resource.c2.C2EProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(nl.utwente.apc.Code2D.resource.c2.C2EProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(nl.utwente.apc.Code2D.resource.c2.C2EProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new nl.utwente.apc.Code2D.resource.c2.util.C2RuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new nl.utwente.apc.Code2D.resource.c2.IC2Command<Object>() {
			public boolean execute(Object context) {
				try {
					...
					nl.utwente.apc.Code2D.resource.c2.mopp.C2Plugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
