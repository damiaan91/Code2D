...
public class C2DebugThread extends nl.utwente.apc.Code2D.resource.c2.debug.C2DebugElement implements org.eclipse.debug.core.model.IThread, nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener {
	
	private boolean suspended = false;
	private nl.utwente.apc.Code2D.resource.c2.debug.C2DebugTarget debugTarget;
	
	public C2DebugThread(nl.utwente.apc.Code2D.resource.c2.debug.C2DebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			nl.utwente.apc.Code2D.resource.c2.debug.C2DebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = nl.utwente.apc.Code2D.resource.c2.util.C2StringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new nl.utwente.apc.Code2D.resource.c2.debug.C2StackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public nl.utwente.apc.Code2D.resource.c2.debug.C2DebugTarget getTarget() {
		return debugTarget;
	}
