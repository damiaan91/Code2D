 ...
 */
public class C2DebuggableInterpreter<ResultType, ContextType> extends nl.utwente.apc.Code2D.resource.c2.debug.AbstractC2Debuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private nl.utwente.apc.Code2D.resource.c2.util.AbstractC2Interpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private nl.utwente.apc.Code2D.resource.c2.IC2Command<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public C2DebuggableInterpreter(nl.utwente.apc.Code2D.resource.c2.util.AbstractC2Interpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new nl.utwente.apc.Code2D.resource.c2.IC2InterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = nl.utwente.apc.Code2D.resource.c2.util.C2StringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public nl.utwente.apc.Code2D.resource.c2.util.AbstractC2Interpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		nl.utwente.apc.Code2D.resource.c2.IC2LocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private nl.utwente.apc.Code2D.resource.c2.IC2LocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		nl.utwente.apc.Code2D.resource.c2.IC2Command<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(nl.utwente.apc.Code2D.resource.c2.debug.EC2DebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = nl.utwente.apc.Code2D.resource.c2.util.C2EObjectUtil.getDepth(current);
		stopCondition = new nl.utwente.apc.Code2D.resource.c2.IC2Command<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = nl.utwente.apc.Code2D.resource.c2.util.C2EObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = nl.utwente.apc.Code2D.resource.c2.util.C2EObjectUtil.getDepth(current) - 1;
		stopCondition = new nl.utwente.apc.Code2D.resource.c2.IC2Command<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
