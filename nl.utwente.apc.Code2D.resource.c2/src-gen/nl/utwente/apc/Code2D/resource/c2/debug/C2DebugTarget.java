...
public class C2DebugTarget extends nl.utwente.apc.Code2D.resource.c2.debug.C2DebugElement implements org.eclipse.debug.core.model.IDebugTarget, nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener {
	
	/**
	 * associated system process
	 */
	private nl.utwente.apc.Code2D.resource.c2.debug.C2DebugProcess process;
	...
	private nl.utwente.apc.Code2D.resource.c2.debug.C2DebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener> eventListeners = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener>();
	
	private nl.utwente.apc.Code2D.resource.c2.debug.C2DebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new nl.utwente.apc.Code2D.resource.c2.mopp.C2MetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				nl.utwente.apc.Code2D.resource.c2.debug.C2DebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(nl.utwente.apc.Code2D.resource.c2.debug.C2DebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public C2DebugTarget(nl.utwente.apc.Code2D.resource.c2.debug.C2DebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new nl.utwente.apc.Code2D.resource.c2.debug.C2DebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new nl.utwente.apc.Code2D.resource.c2.debug.C2DebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(nl.utwente.apc.Code2D.resource.c2.debug.IC2DebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new nl.utwente.apc.Code2D.resource.c2.mopp.C2MetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					nl.utwente.apc.Code2D.resource.c2.debug.C2LineBreakpoint lineBreakpoint = (nl.utwente.apc.Code2D.resource.c2.debug.C2LineBreakpoint) breakpoint;
	...
	public void handleMessage(nl.utwente.apc.Code2D.resource.c2.debug.C2DebugMessage message) {
		try {
			if (message.hasType(nl.utwente.apc.Code2D.resource.c2.debug.EC2DebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(nl.utwente.apc.Code2D.resource.c2.debug.EC2DebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(nl.utwente.apc.Code2D.resource.c2.debug.EC2DebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(nl.utwente.apc.Code2D.resource.c2.debug.EC2DebugMessageTypes.RESUMED)) {
	...
	public nl.utwente.apc.Code2D.resource.c2.debug.C2DebugProxy getDebugProxy() {
