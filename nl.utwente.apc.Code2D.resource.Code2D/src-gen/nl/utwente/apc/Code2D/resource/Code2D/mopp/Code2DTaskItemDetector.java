...
/**
 * The Code2DTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * Code2DTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class Code2DTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DTaskItem> foundItems = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DTaskItem(keyword, message, localLine, offset, end));
