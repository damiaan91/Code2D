 ...
 */
public class Code2DMinimalModelHelper {
	
	private final static nl.utwente.apc.Code2D.resource.Code2D.util.Code2DEClassUtil eClassUtil = new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + nl.utwente.apc.Code2D.resource.Code2D.util.Code2DStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = nl.utwente.apc.Code2D.resource.Code2D.util.Code2DListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + nl.utwente.apc.Code2D.resource.Code2D.util.Code2DStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = nl.utwente.apc.Code2D.resource.Code2D.util.Code2DListUtil.castListUnchecked(value);
