...
public abstract class AbstractCode2DDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<nl.utwente.apc.Code2D.resource.Code2D.util.Code2DPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.util.Code2DPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(nl.utwente.apc.Code2D.resource.Code2D.debug.ECode2DDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			nl.utwente.apc.Code2D.resource.Code2D.debug.Code2DDebugMessage message = new nl.utwente.apc.Code2D.resource.Code2D.debug.Code2DDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				nl.utwente.apc.Code2D.resource.Code2D.util.Code2DPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(nl.utwente.apc.Code2D.resource.Code2D.debug.ECode2DDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(nl.utwente.apc.Code2D.resource.Code2D.debug.ECode2DDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(nl.utwente.apc.Code2D.resource.Code2D.debug.ECode2DDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(nl.utwente.apc.Code2D.resource.Code2D.debug.ECode2DDebugMessageTypes.STARTED, true);
			setSuspend(true);
