 ...
 * contains
 * <code>nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class Code2DMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object>> commands = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object>> commandsToProcess = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextDiagnostic diagnostic) {
		final nl.utwente.apc.Code2D.resource.Code2D.ICode2DProblem problem = diagnostic.getProblem();
		nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemSeverity.ERROR) {
					...
					nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextDiagnostic textDiagnostic = (nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<nl.utwente.apc.Code2D.resource.Code2D.ICode2DQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, nl.utwente.apc.Code2D.resource.Code2D.util.Code2DStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemType nextType : nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(nl.utwente.apc.Code2D.resource.Code2D.Code2DEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new nl.utwente.apc.Code2D.resource.Code2D.ICode2DCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
