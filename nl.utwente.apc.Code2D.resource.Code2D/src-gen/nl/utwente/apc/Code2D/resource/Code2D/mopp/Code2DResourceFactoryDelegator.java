...
public class Code2DResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public Code2DResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DRuntimeUtil().isEclipsePlatformAvailable()) {
			new nl.utwente.apc.Code2D.resource.Code2D.util.Code2DEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DResourceFactory());
		}
	}
