 ...
 */
public class Code2DHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionHelper positionHelper = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionHelper();
	...
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DTokenScanner scanner;
	...
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DOccurrence occurrence;
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, nl.utwente.apc.Code2D.resource.Code2D.ICode2DBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public Code2DHighlighting(nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DColorManager colorManager, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DTokenScanner(textResource, colorManager);
		occurrence = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
