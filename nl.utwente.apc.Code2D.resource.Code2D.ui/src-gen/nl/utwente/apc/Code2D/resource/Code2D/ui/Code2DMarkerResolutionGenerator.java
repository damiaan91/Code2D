...
public class Code2DMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(nl.utwente.apc.Code2D.resource.Code2D.ICode2DOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DResource) {
					nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DResource customResource = (nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<nl.utwente.apc.Code2D.resource.Code2D.ICode2DQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final nl.utwente.apc.Code2D.resource.Code2D.ICode2DQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<nl.utwente.apc.Code2D.resource.Code2D.ICode2DQuickFix> getQuickFixes(nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					nl.utwente.apc.Code2D.resource.Code2D.ICode2DQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
