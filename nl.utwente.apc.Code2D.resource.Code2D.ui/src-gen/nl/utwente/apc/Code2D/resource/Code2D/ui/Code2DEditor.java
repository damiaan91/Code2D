...
/**
 * A text editor for 'Code2D' models.
 * <p>
 * This editor has id
 * <code>nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DEditor</code>
 * The editor's context menu has id
 * <code>nl.utwente.apc.Code2D.resource.Code2D.EditorContext</code>.
 * The editor's ruler context menu has id
 * <code>nl.utwente.apc.Code2D.resource.Code2D.EditorRuler</code>.
 * </p>
 */
public class Code2DEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, nl.utwente.apc.Code2D.resource.Code2D.ICode2DResourceProvider, nl.utwente.apc.Code2D.resource.Code2D.ui.ICode2DBracketHandlerProvider, nl.utwente.apc.Code2D.resource.Code2D.ui.ICode2DAnnotationModelProvider {
	
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DCodeFoldingManager codeFoldingManager;
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DBackgroundParsingStrategy bgParsingStrategy = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DBackgroundParsingStrategy();
	private java.util.Collection<nl.utwente.apc.Code2D.resource.Code2D.ICode2DBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.ICode2DBackgroundParsingListener>();
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DColorManager colorManager = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DColorManager();
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DOutlinePage outlinePage;
	...
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPropertySheetPage propertySheetPage;
	...
	private nl.utwente.apc.Code2D.resource.Code2D.ui.ICode2DBracketHandler bracketHandler;
	...
	public Code2DEditor() {
		super();
		setSourceViewerConfiguration(new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), Code2DEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource currentResource = getResource();
				...
				nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("nl.utwente.apc.Code2D.resource.Code2D.EditorContext");
		setRulerContextMenuId("nl.utwente.apc.Code2D.resource.Code2D.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DNature.activate(inputFile.getProject());
		...
		nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource loadedResource = (nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource) {
					setResource((nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the Code2DEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource textResource = (nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource) element.eResource();
			...
			nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextToken token = lexer.getNextToken();
			...
			nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!Code2DEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(nl.utwente.apc.Code2D.resource.Code2D.ICode2DBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (nl.utwente.apc.Code2D.resource.Code2D.ICode2DBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public nl.utwente.apc.Code2D.resource.Code2D.ui.ICode2DBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(nl.utwente.apc.Code2D.resource.Code2D.ui.ICode2DBracketHandler bracketHandler) {
