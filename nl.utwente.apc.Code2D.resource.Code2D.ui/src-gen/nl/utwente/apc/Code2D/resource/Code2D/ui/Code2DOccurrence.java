 ...
 */
public class Code2DOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "nl.utwente.apc.Code2D.resource.Code2D.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "nl.utwente.apc.Code2D.resource.Code2D.ui.occurences.declaration";
	...
	private nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DTokenScanner tokenScanner;
	 ...
	 */
	public Code2DOccurrence(nl.utwente.apc.Code2D.resource.Code2D.ICode2DTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DOccurrence.DECLARATION_ANNOTATION_ID);
	}
