...
public class Code2DBracketSet {
	...
	private java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair {
	 ...
	 */
	private class ClosingListener implements nl.utwente.apc.Code2D.resource.Code2D.ui.ICode2DBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public Code2DBracketSet(nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new nl.utwente.apc.Code2D.resource.Code2D.mopp.Code2DMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair>();
	 ...
	 */
	public nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair getBracketPair(String opening, String closing) {
		for (nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair bracketPair : bracketPairs) {
	...
	public nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair remove(String opening, String closing) {
		for (nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.Code2D.ICode2DBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, nl.utwente.apc.Code2D.resource.Code2D.ui.Code2DPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
