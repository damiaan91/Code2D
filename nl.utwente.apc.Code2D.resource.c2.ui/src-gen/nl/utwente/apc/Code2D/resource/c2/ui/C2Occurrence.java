 ...
 */
public class C2Occurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "nl.utwente.apc.Code2D.resource.c2.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "nl.utwente.apc.Code2D.resource.c2.ui.occurences.declaration";
	...
	private nl.utwente.apc.Code2D.resource.c2.ui.C2TokenScanner tokenScanner;
	...
	private nl.utwente.apc.Code2D.resource.c2.IC2TextResource textResource;
	 ...
	 */
	public C2Occurrence(nl.utwente.apc.Code2D.resource.c2.IC2TextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, nl.utwente.apc.Code2D.resource.c2.ui.C2TokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new nl.utwente.apc.Code2D.resource.c2.mopp.C2MetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(nl.utwente.apc.Code2D.resource.c2.ui.C2BracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, nl.utwente.apc.Code2D.resource.c2.ui.C2PositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, nl.utwente.apc.Code2D.resource.c2.ui.C2PositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, nl.utwente.apc.Code2D.resource.c2.ui.C2PositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == nl.utwente.apc.Code2D.resource.c2.ui.C2PositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(nl.utwente.apc.Code2D.resource.c2.ui.C2Occurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(nl.utwente.apc.Code2D.resource.c2.ui.C2Occurrence.DECLARATION_ANNOTATION_ID);
	}
