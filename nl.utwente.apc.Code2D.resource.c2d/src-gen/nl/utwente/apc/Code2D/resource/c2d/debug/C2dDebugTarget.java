...
public class C2dDebugTarget extends nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugElement implements org.eclipse.debug.core.model.IDebugTarget, nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener {
	
	/**
	 * associated system process
	 */
	private nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugProcess process;
	...
	private nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener> eventListeners = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener>();
	
	private nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new nl.utwente.apc.Code2D.resource.c2d.mopp.C2dMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public C2dDebugTarget(nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new nl.utwente.apc.Code2D.resource.c2d.mopp.C2dMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					nl.utwente.apc.Code2D.resource.c2d.debug.C2dLineBreakpoint lineBreakpoint = (nl.utwente.apc.Code2D.resource.c2d.debug.C2dLineBreakpoint) breakpoint;
	...
	public void handleMessage(nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message) {
		try {
			if (message.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.RESUMED)) {
	...
	public nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugProxy getDebugProxy() {
