...
/**
 * The C2dTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * C2dTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class C2dTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<nl.utwente.apc.Code2D.resource.c2d.mopp.C2dTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<nl.utwente.apc.Code2D.resource.c2d.mopp.C2dTaskItem> foundItems = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.c2d.mopp.C2dTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new nl.utwente.apc.Code2D.resource.c2d.mopp.C2dTaskItem(keyword, message, localLine, offset, end));
