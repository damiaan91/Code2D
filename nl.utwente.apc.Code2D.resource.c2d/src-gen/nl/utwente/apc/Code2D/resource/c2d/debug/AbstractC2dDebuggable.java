...
public abstract class AbstractC2dDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new nl.utwente.apc.Code2D.resource.c2d.util.C2dRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new nl.utwente.apc.Code2D.resource.c2d.util.C2dRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.STARTED, true);
			setSuspend(true);
