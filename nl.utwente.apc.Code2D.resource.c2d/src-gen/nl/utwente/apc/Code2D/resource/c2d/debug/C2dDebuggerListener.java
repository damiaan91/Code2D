 ...
 * code).
 */
public class C2dDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private nl.utwente.apc.Code2D.resource.c2d.debug.AbstractC2dDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Object>>();
	...
	public C2dDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = nl.utwente.apc.Code2D.resource.c2d.util.C2dStringUtil.encode('#', stack);
				nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.GET_VARIABLES)) {
				...
				nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage message = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return nl.utwente.apc.Code2D.resource.c2d.util.C2dStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Object> pair = new nl.utwente.apc.Code2D.resource.c2d.util.C2dPair<String, Object>(name, value);
	...
	public nl.utwente.apc.Code2D.resource.c2d.debug.AbstractC2dDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(nl.utwente.apc.Code2D.resource.c2d.debug.AbstractC2dDebuggable debuggable) {
