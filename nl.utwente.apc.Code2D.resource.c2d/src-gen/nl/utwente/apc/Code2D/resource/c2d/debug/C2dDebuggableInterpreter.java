 ...
 */
public class C2dDebuggableInterpreter<ResultType, ContextType> extends nl.utwente.apc.Code2D.resource.c2d.debug.AbstractC2dDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private nl.utwente.apc.Code2D.resource.c2d.util.AbstractC2dInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private nl.utwente.apc.Code2D.resource.c2d.IC2dCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public C2dDebuggableInterpreter(nl.utwente.apc.Code2D.resource.c2d.util.AbstractC2dInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new nl.utwente.apc.Code2D.resource.c2d.IC2dInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = nl.utwente.apc.Code2D.resource.c2d.util.C2dStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public nl.utwente.apc.Code2D.resource.c2d.util.AbstractC2dInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		nl.utwente.apc.Code2D.resource.c2d.IC2dLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private nl.utwente.apc.Code2D.resource.c2d.IC2dLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		nl.utwente.apc.Code2D.resource.c2d.IC2dCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(nl.utwente.apc.Code2D.resource.c2d.debug.EC2dDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = nl.utwente.apc.Code2D.resource.c2d.util.C2dEObjectUtil.getDepth(current);
		stopCondition = new nl.utwente.apc.Code2D.resource.c2d.IC2dCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = nl.utwente.apc.Code2D.resource.c2d.util.C2dEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = nl.utwente.apc.Code2D.resource.c2d.util.C2dEObjectUtil.getDepth(current) - 1;
		stopCondition = new nl.utwente.apc.Code2D.resource.c2d.IC2dCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
