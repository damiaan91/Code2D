...
public class C2dDebugThread extends nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugElement implements org.eclipse.debug.core.model.IThread, nl.utwente.apc.Code2D.resource.c2d.debug.IC2dDebugEventListener {
	
	private boolean suspended = false;
	private nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugTarget debugTarget;
	
	public C2dDebugThread(nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = nl.utwente.apc.Code2D.resource.c2d.util.C2dStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new nl.utwente.apc.Code2D.resource.c2d.debug.C2dStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public nl.utwente.apc.Code2D.resource.c2d.debug.C2dDebugTarget getTarget() {
		return debugTarget;
	}
